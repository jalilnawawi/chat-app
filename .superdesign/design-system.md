...
> Source of truth: this file mirrors the repository's own `DESIGN.md` (tokens + rules), with product context added on top. The live tokens are declared in `web/src/index.css` under Tailwind v4 `@theme`; there is no `tailwind.config` file.

## Product context

- **What it is:** a self-hosted realtime team chat (1-on-1 DMs and per-project/per-team groups). One Go binary + PostgreSQL; Redis, SeaweedFS, Web Push and SMTP are each optional and features that depend on them disappear cleanly from the UI instead of breaking.
- **Users:** employees of one company, wide age range including older users. Readability is a requirement, not a decoration. Used every day, on desktop and on phones.
- **Success:** people use it daily without losing messages, without duplicates, and without feeling the app is in their way.
- **UI language:** Indonesian. All labels, empty states and errors are Indonesian.

## Key screens

| Screen | Component | Notes |
| --- | --- | --- |
| App shell | `web/src/App.tsx` | one flex row, max-width 1480px, bordered left/right on `lg` |
| Conversation list | `web/src/components/Sidebar.tsx` | own avatar + status, search, new chat, panel buttons, list |
| **Chat (primary)** | `web/src/components/ChatPanel.tsx` | header, pin bar, notices, history log, typing line, notice stack, composer |
| Right panel | `ProfilePanel` / `AkunPanel` / `PreferensiPanel` / `SearchPanel` / `GroupPanel` | one at a time, all on `ui/PanelShell.tsx` |

No router: screens are state, not URLs. On narrow screens the sidebar and the chat column **alternate** in the same column, driven by `activeId`.

## Jobs to be done (chat screen)

1. Read the running conversation and know instantly which messages are mine.
2. Know when someone calls my name (mention) — this is the only job the mango tone has.
3. Send text, emoji, files and voice notes without leaving the write row.
4. Reply to, quote, react to, forward, pin, edit and delete a specific message.
5. Know delivery state: sending, sent, read, failed — and recover from failure.
6. Find my way back: jump to a pinned/searched message, and return to the latest.

## Layout & motion invariants (do not break)

- Reading column is capped at `max-w-[56rem]` and centred; messages stack from the bottom (`justify-end`, `min-h-full`).
- History padding: `px-3 py-3` on mobile, `md:px-6 md:py-4`.
- `#root` is `100dvh`, never `%` — the mobile URL bar must not clip the composer.
- Composer touch target 44px; back-to-latest pill is `sticky bottom-1`, teal, with `shadow-pop`.
- Flat at rest. Shadow (`--shadow-pop`) only for things that float: menus, dialogs, the back-to-latest pill.
- Only two idle animations exist: `denyut` (typing dots) and `rekam` (recording dot). Nothing else may animate on its own.
- Single app-wide focus ring (`:focus-visible` → 2px `--color-accent`); `.cincin-sendiri` opts out, `.sasaran-fokus` moves the ring onto the bubble, `.aksi-pesan` must become visible on keyboard focus.
- Hover-only affordances (message menu arrow, react button) must also appear on keyboard focus.
- WCAG 2.2 AA is a hard floor in both themes.
